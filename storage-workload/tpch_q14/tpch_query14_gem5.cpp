...
void tpch_query14_baseline(char *date, int fd, void *buffer, unsigned int pageCount1, unsigned int pageCount2, double & promo_revenue, double & revenue)
{
	...
	exec_time_t et_io1, et_io2;
	char et_name[64];
	strcpy(et_name, "IO1");
	init_exec_time(&et_io1, et_name);
	strcpy(et_name, "IO2");
	init_exec_time(&et_io2, et_name);

	exec_time_t et_calc1, et_calc2;
	strcpy(et_name, "calc1");
	init_exec_time(&et_calc1, et_name);
	strcpy(et_name, "calc2");
	init_exec_time(&et_calc2, et_name);
	...
	for(unsigned int i2 = pageCount1; i2 < pageCount2; i2+=NUMBER_OF_PAGES_IN_A_COMMAND)
	{
		...
		set_start_time(&et_io1);
		my_sleep(FLASH_ACCESS_TIME);
		add_exec_time(&et_io1);

		set_start_time(&et_calc1);
...
//				std::string p_type((((char *)buffer)+4096*j2+offset2+98) ,(((char *)buffer)+4096*j2+offset2+103));
		...
		add_exec_time(&et_calc1);
	}

	print_exec_time(&et_io1);
	print_exec_time(&et_calc1);
	...
	for(unsigned int i=0; i < pageCount1; i += NUMBER_OF_PAGES_IN_A_COMMAND)
	{
		...
		set_start_time(&et_io2);
		my_sleep(FLASH_ACCESS_TIME);
		add_exec_time(&et_io2);

		set_start_time(&et_calc2);
		...
		add_exec_time(&et_calc2);
	}

	print_exec_time(&et_io2);
	print_exec_time(&et_calc2);
